...
def get_device_mesh(distributed_args: DistributedArgs, device: torch.device):
    ...
    logging.info(
        f"Creating device mesh with dp_replicate={dp_replicate}, dp_shard={dp_shard}, tp_size={tp_size} in world_size={get_world_size()}")
...
def check_model_value_range(
    ...
    for name, param in chain(model.named_parameters(), model.named_buffers()):
        try:
            ...
            logger.warning(
                f"Model parameter {name} is not a tensor: {e}. This is probably because of FSDP sharding"
            )
            continue
...
def parallelize_model(
    ...
    if (
        ...
        logging.info(
            f"FSDP config: {fsdp_config}, fsdp_type: {distributed_args.fsdp_type}, using mesh: {device_mesh}"
        )
        ...
        logging.info(
            f"FSDP grouping plan: {fsdp_grouping_plan}, fsdp_type: {distributed_args.fsdp_type}"
        )

        for path, reshard_after_forward in fsdp_grouping_plan:
            # print(path, reshard_after_forward, fsdp_grouping_plan)
